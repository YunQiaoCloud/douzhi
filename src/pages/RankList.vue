<template>
  <div class="movie-list">
    ...
    <div v-if="movie.id" v-for="movie in rankList" :key="movie.id" class="movie-list-item">
      <router-link :to="{ name: 'MovieDetails', params: { id: movie.id } }">
        <div class="cover" :style="{ 'background-image': `url(${movie.images.small})` }"></div>
      </router-link>
      <p class="name">{{movie.title}}</p>
      <p class="pubdate"
         v-for="pubdates in movie.pubdates" :key="pubdates.id">{{pubdates}}</p>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    rankList() {
      // 计算属性返回 store 里的数据
      return this.$store.state.ranks.list
    }
  },
  async created() {
    // store 执行获取 rankList
